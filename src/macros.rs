...
mod test {
    ...
    fn syntax() {
        ...
        assert_eq!(t!(5p).akaize().as_u8(), tu8!(5pr));

        assert_eq!(tu8![8p,], [tu8!(8p)]);
        assert_eq!(tuz![P,], [tuz!(P)]);
        assert_eq!(t![N,], [t!(N)]);

        assert_eq!(tu8![2p, 5pr, S], [tu8!(2p), tu8!(5pr), tu8!(S)]);
        assert_eq!(tuz![E, 6s, ?], [tuz!(E), tuz!(6s), tuz!(?)]);
        assert_eq!(t![1m, 2p, 9s], [t!(1m), t!(2p), t!(9s)]);

        assert!(matches_tu8!(t!(E).as_u8(), 1m | E | ? | 5pr));
        assert!(!matches_tu8!(t!(3s).as_u8(), 1s | 7p | P));
    }
