...
impl Tile {
    ...
    pub const fn next(self) -> Self {
        ...
        if kind < 2 {
            ...
            Self(2 * 9 + 4 + (num - 4 + 1) % 3)
        } else {
            Self(2 * 9 + 7 + (num - 7 + 1) % 2)
        }
    }
    ...
    pub const fn prev(self) -> Self {
        ...
        if kind < 2 {
            ...
            Self(2 * 9 + 4 + (num - 4 + 3 - 1) % 3)
        } else {
            Self(2 * 9 + 7 + (num - 7 + 2 - 1) % 2)
        }
    }
}
...
mod test {
    ...
    #[test]
    fn next_prev() {
        MJAI_PAI_STRINGS.iter().take(30).for_each(|&s| {
